...
public class FrameServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request,
		...
		String characterEncoding = frameConfigration.getConfigParam("characterEncoding");
		request.setCharacterEncoding(characterEncoding);
		response.setCharacterEncoding(characterEncoding);
		response.setContentType(frameConfigration.getConfigParam("contentType"));
		Object[] route = parse(request);
		try {
			if (null == route) throw new Exception("uriError");
			processInvoke(request, response, route, null, 0);
		} catch (Exception e) {
			route[0] = frameConfigration.getConfigParam("defaultErrorController");
			route[1] = frameConfigration.getConfigParam("defaultErrorAction");
			try {
	...
	private void processInvoke(HttpServletRequest request, HttpServletResponse response,
		...
		Class<?> controller = Class.forName(frameConfigration.getConfigParam("classNamePath")
				+ "." + controllerName.substring(0, 1).toUpperCase()
				+ controllerName.substring(1)
				+ frameConfigration.getConfigParam("defaultControllerSuffix"));
		...
		if (null == e) {
			actionVal = controller.getMethod(actionName
					+ frameConfigration.getConfigParam("defaultActionSuffix")).invoke(instance);
		} else {
			actionVal = controller.getMethod(
					actionName + frameConfigration.getConfigParam("defaultActionSuffix"),
	...
	private void proessError(HttpServletRequest request,
		...
		if (count > 0) {
			Log logger = LogFactory.getLog(FrameConfigration.class);
	...
	private Object[] parse(HttpServletRequest request) {
		...
		route[0] = frameConfigration.getConfigParam("defaultControllerName"); // controllerName
